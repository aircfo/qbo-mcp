...
**Where it is built and proven.** The Railway project **"QBO MCP (Sandbox)"** (same code, `INTUIT_ENVIRONMENT=sandbox`) is the write-development target; the September plan's "sandbox connector is for writes only and is not in the repo" becomes a `DEPLOY.md` section. Dress rehearsal 09-28→09-30: commit on sandbox, dry-run against airCFO production, Kim reads the dry-run output once before the close.
...
| **P3 · Writes** | §4: `categorize_transactions`, `create_journal_entries`, approval tokens, `write_audit`, `get_write_log`, sandbox deploy documented, tests for every guardrail | 10-02 mapping day (imports); JE drafts posted by Claude once Kim's tolerance is written | 2–3 | Wed 09-23 → Mon 09-28 (dress rehearsal 09-28 → 09-30) |
