...
| `src/tools/reports.ts` | `get_transaction_list` description says plainly that `cleared: "Uncleared"` with an `account` filter is the uncleared-items list (G10). **Corrected 2026-09-12 (`fix/pre-launch-review`):** Intuit's TransactionList model has no `account` parameter at all, so the description now says so and points at `source_account_type` plus `group_by: Account` instead |
...
| `src/server.ts` | write tools registered only when `env.WRITES_ENABLED` **and** the connection's `writes_enabled` **and** `email_verified` is set. **Registration is a courtesy to the tool list, not the guard** (Codex review 2026-09-12, #5): a session outlives any switch flip, so every write handler re-reads the global switch, the connection's flag from the store and the session's verified identity **at call time**, before the approval token is even checked. A switch turned off mid-session takes effect on the next call, not the next session |
| `DEPLOY.md` | how to enable writes on one connection (`set_writes_enabled`) and how to turn them all off (`WRITES_ENABLED=false`) |
| tests | `_writes.test.ts`: canonical form stable across key order and whitespace; token bound to connection, email and hash, expires, single-use; unbalanced entry refused; `Payment` / `Transfer` / `BillPayment` refused by type; funding line refused; the sparse payload carries every line. `writes.test.ts` with a fake `qb`: read-modify-write sends the SyncToken; a SyncToken conflict fails that item and the batch continues; a replayed `batch_key` returns the stored result and posts nothing; dry-run writes an audit row too; a session opened while writes were enabled is refused on its next call after `writes_enabled` is turned off; `mode: commit` without a verified identity is refused before any QuickBooks read |
...
bearer, verified on the sandbox first; G10 `get_uncleared_transactions(account, as_of)` (note: TransactionList has no `account` param — filter by `source_account_type` and select the account from the grouped rows server-side); G11
