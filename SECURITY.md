...
- **Logging/audit.** One structured `mcp_request` line per call
  ...
  events logged with the self-reported email. Railway captures stdout/stderr.
...
3. **Terms of service**, linked from the connect page via `TERMS_URL` /
   `PRIVACY_URL`.
4. **A security contact / disclosure path** (e.g. security@yourdomain).

## Reporting a vulnerability

Email the security contact above with details and reproduction steps. Please do
